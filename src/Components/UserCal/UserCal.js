...
export default function UserCal(props) {
  const classDB = [
    ...
    { Class: "LQ4", Day: "Wednesday", Time: "17:00", Note: "No students" }
  ];
            ...
            {columns.map(day =>
              props.classDB.filter(
                ...
                <td style={{ backgroundColor: "lightgreen" }}>
                  {props.classDB.filter(aClass => aClass.Day === day)[0].Class}
                  {props.classDB.filter(aClass => aClass.Day === day)[0].Note===undefined?null:<img onClick={()=>alert(props.classDB.filter(aClass => aClass.Day === day)[0].Note)} src={star} alt="star"/>}
                </td>
              )
            )}
          </tr>
